...
class XsltProcess
{
    ...
    private function __process(XsltProcessor $XSLProc, $xml, $xsl, array $parameters = array())
    {
        ...
        // Libxml Errors check by tiloschroeder
        $errors = libxml_get_errors();
        if (!empty($errors)) {
            foreach ($errors as $error) {
                // Write to Symphony Log:
                Symphony::Log()->writeToLog("[XSLT Import Error] Line {$error->line}: {$error->message}", E_ERROR, true);
            }

            libxml_clear_errors();

            // Cancel so that transformToXML() does not throw the generic error
