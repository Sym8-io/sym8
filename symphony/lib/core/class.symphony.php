...
abstract class Symphony implements Singleton
{
    ...
    public static function getMigrationVersion()
    {
        if (self::isInstallerAvailable()) {
            // More robuist migration filter: scan only mirgration files and sort them
            // added by tiloschroeder.de
            $migrations = array_filter(
                scandir(DOCROOT . '/install/migrations'),
                function ($file) {
                    return preg_match('/^\d+\.\d+\.\d+\.php$/', $file);
                }
            );
            ...
            require_once(DOCROOT . '/install/migrations/' . $migration_file); // <--- that was missing!
...
class DatabaseExceptionHandler extends GenericExceptionHandler
{
    ...
    public static function render($e)
    {
        ...
        $html = str_replace('{VERSION}', Symphony::Configuration()->get('version', 'symphony'), $html);
